(function (global) {
  ...
  var localKey = "ova_u2_03_scorm_data";
  var localData = {};
  ...
  function initialize() {
    ...
    if (api) {
      ...
      if (!initialized) console.warn("SCORM: LMSInitialize no respondio correctamente.");
    } else {
      initialized = true;
      console.info("SCORM API no encontrada. OVA ejecutandose en modo local.");
    }
    return initialized;
  }
  ...
  function resetLocal() {
    localData = {};
    try { localStorage.removeItem(localKey); } catch (error) { /* Sin accion. */ }
  }
